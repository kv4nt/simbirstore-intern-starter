      ...
      <n-spin :show="loading">
        ...
        <div v-for="order in orders" :key="order.id" class="ss-order-card">
          ...
          <div v-if="order.items && order.items.length" style="display:flex;flex-direction:column;gap:8px">
            <div v-for="item in order.items" :key="item.id" style="display:flex;justify-content:space-between;font-size:14px;padding:6px 0;border-bottom:1px solid var(--ss-border)">
              <span>{{ item.name }} <span v-if="item.size" style="color:var(--ss-muted)"> · {{ item.size }}</span> <span style="color:var(--ss-muted)"> × {{ item.quantity }}</span></span>
